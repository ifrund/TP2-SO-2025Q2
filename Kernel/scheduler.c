    ...
    if (current_index >= 0 && process_table[current_index] != NULL)
    {
        ...
        if (curr->state == RUNNING)
        {
            ...
            if (curr->time_used >= curr->my_max_time)
            {
                ...
                { // Si sos el idle te volvemos a bloquear, sino queda ready
                    curr->state = BLOCKED;
                }
                else
                {
...
{ // no se puede usar yield al final de kill, porq el get_pid devuelve -1 ya q el proceos esta ZOMBIE y no RUNNING
