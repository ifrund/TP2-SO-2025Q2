...
//nuestro init tmb es nuesto idle, y recibira a hijos huerfanos
static void init(){
	char * argNull[1]={NULL};
	create_process(shell, "shell", 0, argNull); //es el segundo, se crea con pid 1
	while(1){
		_hlt();
	}
}
...
{	
	...
	//TODO init de pipes (?
	char * argNull[1]={NULL};
	create_process(init, "init", 0, argNull); //es el primero, se crea con pid 0
	current_index = 0;
	create_process(shell, "shell", 0, argNull); //es el segundo, se crea con pid 1
